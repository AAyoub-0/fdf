...
/*   Updated: 2025/02/24 17:30:15 by aboumall         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	no_event(void *data)
{
	...
	mlx = (t_mlx *)data;
	mlx_mouse_get_pos(mlx->mlx, mlx->win, &mlx->mouse->x, &mlx->mouse->y);
	if (mlx->ins)
	{
		menu_frame_event_hover(mlx);
		close_btn_event_hover(mlx);
		...
		rot_event_hover(mlx);
		proj_event_hover(mlx);
		map_btn_event_hover(mlx);
		color_btn_event_hover(mlx);
	}
